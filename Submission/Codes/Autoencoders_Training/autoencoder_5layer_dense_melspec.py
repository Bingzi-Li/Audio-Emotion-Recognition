...
if encoding_dim == 100:
	...
	encoded = layers.Dense(6000, activation='relu')(input_img)
	...
	decoded = layers.Dense(6000, activation='relu')(decoded)
	if dropout:
...
if dropout:
	encoder.save('./autoencoder_loss/5_layer_encoder-{}-dropout={}-{}-{}-{}.hdf5'.format(cell,dropout,optimizer,feature,encoding_dim))
else:
	encoder.save('./autoencoder_loss/5_layer_encoder-no_dropout={}-{}-{}-{}.hdf5'.format(cell,optimizer,feature,encoding_dim))
import pickle
res = {"loss":history.history["loss"],"val_loss" : history.history["val_loss"]}
if dropout:
	pickle.dump(res,open("./autoencoder_loss/5_layer_encoder-{}-dropout={}-{}-{}-{}.pkl".format(cell,dropout,optimizer,feature,encoding_dim),"wb"))
else:
	pickle.dump(res,open("./autoencoder_loss/5_layer_encoder-no_dropout-abs{}-{}-{}-{}.pkl".format(cell,optimizer,feature,encoding_dim),"wb"))
